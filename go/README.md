...
The Qwen Image Go SDK is the language-specific package for Qwen Image on RunAPI. Use this package for image generation, image editing, and creative production workflows when your application needs request bodies, task status lookup, and consistent RunAPI errors in Go.

This README is the Go package guide inside the public `qwen-image-sdk` repository. For the repository overview, start at `../README.md`; for model details, use https://runapi.ai/models/qwen-image; for API reference, use https://runapi.ai/docs#qwen-image; for SDK docs, use https://runapi.ai/docs#sdk-qwen-image.
...
client, err := qwenimage.NewClient()
task, err := client.TextToImage.Create(context.Background(), qwenimage.TextToImageParams{
