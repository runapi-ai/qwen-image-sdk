...
RSpec.describe RunApi::QwenImage::Client do
  before do
  ...
  context "with custom http_client" do
    ...
    it "falls back to Core::HttpClient when http_client is nil" do
      ...
      expect(client.edit_image.instance_variable_get(:@http)).to be_a(RunApi::Core::HttpClient)
    end
  end

  it "exposes resource accessors" do
    client = described_class.new(api_key: "test-key")
    expect(client.text_to_image).to be_a(RunApi::QwenImage::Resources::TextToImage)
    expect(client.remix_image).to be_a(RunApi::QwenImage::Resources::RemixImage)
    expect(client.edit_image).to be_a(RunApi::QwenImage::Resources::EditImage)
  end
end
